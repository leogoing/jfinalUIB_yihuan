			...
			if(info!=undefined){
				...
				$.each(info, function( index, value ){
					var div = $('<div id="'+value.id+'" class="more more32"></div>' )
						.css('top',value.top).css('left',value.left).css('position','absolute').appendTo(picture);
					var imgButton;
					if(self.options.modify){
/*3:07*/					  imgButton = $('<div class="'+value.type+'" title="drag in the picture"></div>').appendTo(div);
...
//animation for tooltips
	...
	initialization: function(){
		...
		$.each(self.options.pictures, function( index, value ) {
			...
			var listContainer = $('<div class="listContainer"><ul><li class="'+value+'-button '+self.options.button+
					'" title="drag in the picture"></li></ul><div class="bgList" title="choose another background for tooltips"></div><div class="buttonsList" title="choose another color for tooltips"></div></div>')
						.prependTo('#'+value);
			...
			$('#'+value).droppable({
				...
				drop: function( event, ui ) {
					...
					var div = $('<div id="'+value+'-more'+plus+'" class="'+value+' more more32"></div>' )
						.css('top',event.pageY-picture.position().top+"px").css('left',ui.position.left+"px").draggable(
						//{containment: picture}
						).css('position','absolute').appendTo(picture);
					//add selected animationBg if animation set true
		      if(self.options.animation){
		        $(".more").addClass(self.options.animationBg);
		      }else{
		...
		*/
		...
		if(self.options.modify){
			//$('<div class="buttonSave"><p>修改模式</p><input type="button" value="save" class="save" title="get code!"/></div>').prependTo(self.element);
			$('<div class="buttonSave"><p>修改模式</p><input type="button" value="save" class="save" title="get code!"/></div>').appendTo('.slide');
		}
...
/*12:06*/				moreInfos = moreInfos+'{"id":"'+value.id+'","type":"'+$(value).find('.imgButtonDrag').attr('class')+'","descr":"'+descr+'","top":"';
		...
		if(self.options.animation){
		  ...
		  //alert('animation: true, animationType: "'+self.options.animationType+'", animationBg: "'+self.options.animationBg+'", button: "'+self.options.button+'", '+moreInfos);
			...
			alert(json);
				$.ajax({
					type : "post",
					url : "unitpic/getdes",//这里的saveData是处理数据，保存数据的action
					data : {"des":json,"ids":ids},
					async : false,
					dataType : "JSON",
