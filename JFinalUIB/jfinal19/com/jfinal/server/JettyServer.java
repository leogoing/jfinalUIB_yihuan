...
class JettyServer implements IServer {
	...
	private void doStart() throws IOException {
		...
		// configureScanner，开启定时器循环扫描重新部署
	...
	private static boolean available(int port) {
		...
		try {
			ss = new ServerSocket(port);//创建服务接收请求（TCP）
			ss.setReuseAddress(true);//设置请求在等待状态时可以复用该端口：tcp状态为TIME_WAIT时可以再new一个socket继续使用相同的端口，如（tcp协议可能需要多次确认连接状态，关闭连接时实际上没有关闭完还要等待会，但是又立即重启创建了新连接，将该属性设置为true时就可以允许这样做：重启服务器）
			ds = new DatagramSocket(port);//用来发送和接收数据报包的套接字(UDP)
