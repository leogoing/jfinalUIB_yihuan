...
import little.ant.platform.beetl.func.Decode;
...
//		}
		...
		groupTemplate.registerFunction("Decode", new Decode()); // 中文解码
	...
	public void configPlugin(Plugins me) {
		 ...
		 */
		/**
		log.info("configPlugin 配置辅助Druid数据库连接池连接属性");
		DruidPlugin druidPlugin2 = new DruidPlugin(
				"jdbc:sqlite:WebContent/sqlite_db/bookmarks2Units.db", "", "", 
				"org.sqlite.JDBC");

		log.info("configPlugin 配置辅助Druid数据库连接池大小");
		druidPlugin2.set(
				(Integer)PropertiesPlugin.getParamMapValue(DictKeys.db_initialSize), 
				(Integer)PropertiesPlugin.getParamMapValue(DictKeys.db_minIdle), 
				(Integer)PropertiesPlugin.getParamMapValue(DictKeys.db_maxActive));
		log.info("configPlugin 配置辅助ActiveRecord插件");
		ActiveRecordPlugin arpAssisc = new ActiveRecordPlugin(DictKeys.db_dataSource_assisc, druidPlugin2);
		arpAssisc.setDevMode(getPropertyToBoolean(DictKeys.config_devMode, false)); // 设置开发模式
		arpAssisc.setShowSql(getPropertyToBoolean(DictKeys.config_devMode, false)); // 是否显示SQL
		
		log.info("configPlugin 使用辅助数据库 sqlite");
		arpAssisc.setDialect(new Sqlite3Dialect());
		arpAssisc.setContainerFactory(new CaseInsensitiveContainerFactory(true));// 配置属性名(字段名)大小写不敏感容器工厂
		*/
		
		log.info("configPlugin 添加druidPlugin插件");
		me.add(druidPlugin)/*.add(druidPlugin2)*/; // 多数据源继续添加
		...
		//arpMap.put(DictKeys.db_dataSource_assisc, arpAssisc);// 多数据源继续添加
		new TablePlugin(arpMap).start();
		me.add(arpMain)/*.add(arpAssisc)*/; // 多数据源继续添加
	...
	public void afterJFinalStart() {
		...
		dirPaths.add("F:\\workSpaceToEclipse\\JFinalUIB\\WebContent\\sqlite_db");
		ToolUtils.startDelTimer(60*60*24*1000, 3000, dirPaths, "");
	}
