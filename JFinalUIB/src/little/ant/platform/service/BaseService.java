...
public abstract class BaseService {
	 ...
	 * @author 李望
	 */
	protected void splitPageBase4In(String dataSource, SplitPage splitPage, String select, String sqlExceptSelect,Object[] paras){
		StringBuilder formSqlSb=new StringBuilder(sqlExceptSelect);
		// 行级：过滤
		rowFilter(formSqlSb);
		...
		String formSql = formSqlSb.toString();
		log.debug("分页查询要执行的sql语句后半部formsql："+formSql);
		Page<?> page = Db.use(dataSource).paginate(splitPage.getPageNumber(), splitPage.getPageSize(), select, formSql, paras);
		splitPage.setTotalPage(page.getTotalPage());
		splitPage.setTotalRow(page.getTotalRow());
		splitPage.setList(page.getList());
		splitPage.compute();
