...
public class IndexService extends BaseService {
	...
	public List<Menu> menu(String systemsIds, User user, String i18n){
		...
		List<Menu> oneList = Menu.dao.find(getSql("platform.menu.child", param), parentmenuids); // 找出根下一级的所有菜单
		param.put("fitler", fitler); // 过滤没有权限的菜单
		for (Menu oneMenu : oneList) { // 找出每个一级菜单的子菜单
			// 二级菜单
